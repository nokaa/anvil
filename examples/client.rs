        ...
        match self {
            ...
            Tcp::Reading(mut sock) => {
                ...
                match sock.try_read(&mut data) {
                    ...
                    Ok(None) => Response::ok(Tcp::Reading(sock)),
                }
            }
        }
    }
...
fn main() {
...

    {
        ...
        ap.set_description("
            ...
            Note this \
                            relies on the ability to always do successful writes to
            \
                            the socket (which is true for small interactive sessions). Do not
            \
                            use it as a real telnet client.
            ");
        ap.refer(&mut host).add_argument("ip",
                                         Store,
                                         "
            The IP addresss to connect to. No name \
                                          resolution here.");
        ap.refer(&mut port).add_argument("port",
                                         Store,
                                         "
            Port to connect to. Default is 3000 which \
                                          is the port of
    ...
    let conn = TcpStream::connect(// Any better way for current stable rust?
